...
export default function MatchEdit() {
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6">
      <div>
        <Link to={`/matches/${id}`} className="text-gray-400 hover:text-white text-sm flex items-center gap-1 mb-2">
          ← Back to Match
        </Link>
        <h1 className="text-2xl font-bold text-white">Edit Match</h1>
        <p className="text-gray-400 mt-1">Update match details</p>
      </div>
      ...
      <div className="bg-[#1a1a2e] border border-gray-800 rounded-xl p-6 shadow-xl">
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <input
              type="text" name="opponent" value={formData.opponent} onChange={handleChange}
          ...
          <div>
            ...
            <input
              type="date" name="matchDate" value={formData.matchDate} onChange={handleChange}
              className="w-full px-4 py-3 bg-[#111111] border border-gray-700 rounded-lg text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary"
            />
          </div>
          ...
          <div className="flex items-center gap-3">
            <input
              type="checkbox" name="isCompleted" checked={formData.isCompleted} onChange={handleChange}
          ...
          {formData.isCompleted && (
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" name="homeScore" value={formData.homeScore} onChange={handleChange}
                  className="w-full px-4 py-3 bg-[#111111] border border-gray-700 rounded-lg text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary"
                  min="0" required
                />
              </div>
              <div>
                ...
                <input type="number" name="awayScore" value={formData.awayScore} onChange={handleChange}
                  className="w-full px-4 py-3 bg-[#111111] border border-gray-700 rounded-lg text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary"
                  min="0" required
                />
              </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="button" onClick={() => navigate(`/matches/${id}`)}
